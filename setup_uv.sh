...
PROJECT_DIR="${SCRIPT_DIR}/week2/workspace/twin/backend"
...
uv pip install --trusted-host pypi.org --trusted-host files.pythonhosted.org certifi

echo -e "${GREEN}==> Finding certifi cacert.pem location...${NC}"
CACERT_PATH=$(uv run python -c "import certifi; print(certifi.where())")
